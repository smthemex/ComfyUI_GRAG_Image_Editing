...
from .lora_pipeline import QwenImageLoraLoaderMixin
...
class QwenImageEditPlusPipeline(DiffusionPipeline, QwenImageLoraLoaderMixin):
    ...
    def prepare_latents(
        ...
        if image_latents is None:
            ...
            all_image_latents = []
            for lats in image_latents:
                lats=lats.to(device=device, dtype=dtype)
                latents_mean = (
                    torch.tensor(vae_config_sm["latents_mean"])
                    .view(1, 16, 1, 1, 1)
                    .to(image_latents.device, image_latents.dtype)
                )
                latents_std = (
                    torch.tensor(vae_config_sm["latents_std"])
                    .view(1, self.latent_channels, 1, 1, 1)
                    .to(image_latents.device, image_latents.dtype)
                )
                lats = (lats - latents_mean) / latents_std
                image_latent_height, image_latent_width = lats.shape[3:]
                # print(image_latents.shape)
                lats = self._pack_latents(
                    lats, batch_size, num_channels_latents, image_latent_height, image_latent_width)
                all_image_latents.append(lats)
            image_latents = torch.cat(all_image_latents, dim=1)
    ...
    def __call__(
        ...
        calculated_width, calculated_height=image_latents[0].shape[4]*8,image_latents[0].shape[3]*8
        ...
        if prompt is not None :
            ...
            prompt_embeds_mask = torch.ones(batch_size, seq_len, dtype=torch.long, device=device)
            prompt_embeds_mask = prompt_embeds_mask.repeat(1, 1, 1)
            prompt_embeds_mask = prompt_embeds_mask.view(batch_size * 1, seq_len)
            if do_true_cfg:
                batch_size, seq_len, _ = negative_prompt_embeds.shape
                negative_prompt_embeds_mask = torch.ones(batch_size, seq_len, dtype=torch.long, device=device)
                negative_prompt_embeds_mask = negative_prompt_embeds_mask.repeat(1, 1, 1)
                negative_prompt_embeds_mask = negative_prompt_embeds_mask.view(batch_size * 1, seq_len)
        ...
        if output_type == "latent":
            ...
            latents_std = 1.0 / torch.tensor(vae_config_sm["latents_std"]).view(1,16, 1, 1, 1).to(
        ...
        if output_type != "latent":
            for x0_latents in x0_previews:
                ...
                x0_latents_std = 1.0 / torch.tensor(vae_config_sm["latents_std"]).view(1, 16, 1, 1, 1).to(
            ...
            for x0_latents in x0_previews:
                x0_latents = self._unpack_latents(x0_latents, height, width, self.vae_scale_factor)
                x0_latents = x0_latents.to(self.transformer.dtype)
                x0_latents_mean = (
                    torch.tensor(vae_config_sm["latents_mean"])
                    .view(1, 16, 1, 1, 1)
                    .to(x0_latents.device, x0_latents.dtype)
